...
__export(main_exports, {
  VIEW_TYPE_TREEMAP: () => VIEW_TYPE_TREEMAP,
  default: () => DigitalGardenTreemapPlugin
});
...
var DigitalGardenTreemapPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
...
var TreemapView = class extends import_obsidian.ItemView {
  ...
  renderControls(container) {
    ...
    (0, import_obsidian.setIcon)(privacyToggle, this.plugin.settings.showTitle ? "eye" : "eye-off");
    ...
    const createSegment = (value, labelKey) => {
      ...
      segment.onclick = async () => {
        if (this.plugin.settings.sizingStrategy === value)
          return;
  ...
  async renderTreemap(container) {
    ...
    const nodes = svg.selectAll("g").data(root2.descendants()).join(
      ...
      (exit) => exit.call((exit2) => exit2.transition(transition2).style("opacity", 0).remove())
    );
    nodes.append("rect").attr("width", (d) => Math.max(0, d.x1 - d.x0)).attr("height", (d) => Math.max(0, d.y1 - d.y0)).attr("rx", (d) => d.depth > 0 && d.depth < 3 ? 4 : 0).attr("fill", (d) => this.getNodeColor(d)).attr("stroke", (d) => d.depth > 0 && d.depth < 3 ? "rgba(180, 180, 180, 0.6)" : "rgba(255, 255, 255, 0.05)").attr("stroke-width", (d) => d.depth > 0 && d.depth < 3 ? "0.8px" : "0.5px").style("cursor", "pointer").attr("class", (d) => this.getNodeClass(d)).on("mouseenter", (event, d) => {
  ...
  getNodeColor(d) {
    ...
    const diffDays = (now2 - mtime) / (1e3 * 60 * 60 * 24);
  ...
  async processFolder(folder, depth) {
    ...
    (_c = node.children) == null ? void 0 : _c.forEach(countRecursive);
...
var DigitalGardenSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName(this.plugin.t("show_heatmap_label")).setDesc(this.plugin.t("show_heatmap_desc")).addToggle((toggle) => toggle.setValue(this.plugin.settings.showHeatmap).onChange(async (value) => {
    ...
    if (this.plugin.settings.palette === "custom") {
      new import_obsidian.Setting(containerEl).setName(this.plugin.t("fresh_color_label")).setDesc(this.plugin.t("fresh_color_desc")).addColorPicker((color2) => color2.setValue(this.plugin.settings.freshColor).onChange(async (value) => {
        this.plugin.settings.freshColor = value;
        await this.plugin.saveSettings();
      }));
      new import_obsidian.Setting(containerEl).setName(this.plugin.t("growth_color_label")).setDesc(this.plugin.t("growth_color_desc")).addColorPicker((color2) => color2.setValue(this.plugin.settings.growthColor).onChange(async (value) => {
        this.plugin.settings.growthColor = value;
        await this.plugin.saveSettings();
      }));
      new import_obsidian.Setting(containerEl).setName(this.plugin.t("base_color_label")).setDesc(this.plugin.t("base_color_desc")).addColorPicker((color2) => color2.setValue(this.plugin.settings.baseColor).onChange(async (value) => {
        this.plugin.settings.baseColor = value;
        await this.plugin.saveSettings();
      }));
    }
    new import_obsidian.Setting(containerEl).setName(this.plugin.t("max_depth_label")).setDesc(this.plugin.t("max_depth_desc")).addSlider((slider) => slider.setLimits(3, 6, 1).setValue(this.plugin.settings.maxDepth).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.maxDepth = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.t("new_file_days_label")).setDesc(this.plugin.t("new_file_days_desc")).addSlider((slider) => slider.setLimits(1, 30, 1).setValue(this.plugin.settings.newFileDaysThreshold || 7).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.newFileDaysThreshold = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName(this.plugin.t("excluded_folders_label")).setDesc(this.plugin.t("excluded_folders_desc")).addButton((btn) => btn.setButtonText(this.plugin.t("add_folder_btn")).setCta().onClick(async () => {
    ...
    this.plugin.settings.excludedFolders.forEach((rule, index) => {
      new import_obsidian.Setting(excludedContainer).addDropdown((dropdown) => dropdown.addOption("path", this.plugin.t("mode_path")).addOption("keyword", this.plugin.t("mode_keyword")).addOption("extension", this.plugin.t("mode_extension")).setValue(rule.mode).onChange(async (value) => {
        this.plugin.settings.excludedFolders[index].mode = value;
        await this.plugin.saveSettings();
      })).addText((text) => text.setPlaceholder(rule.mode === "extension" ? "e.g. canvas" : "path/to/folder").setValue(rule.value).onChange(async (value) => {
        this.plugin.settings.excludedFolders[index].value = value;
        await this.plugin.saveSettings();
      })).addExtraButton((btn) => btn.setIcon("trash").setTooltip(this.plugin.t("remove_btn")).onClick(async () => {
